    ...
    <section className="py-24 px-6 w-full max-w-7xl mx-auto" id="stack">
      <div className="flex flex-col gap-4 mb-16 text-left">
        <h2 className="text-4xl md:text-5xl font-bold text-white tracking-tight">
          Mi <span className="text-blue-500">Stack Tecnológico</span>
        </h2>
        <p className="text-gray-400 text-lg max-w-2xl">
          Especialización en desarrollo Full Stack y soluciones impulsadas por Inteligencia Artificial.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {TECH_STACK.map((group, index) => (
          <div 
            key={index} 
            className="group bg-[#0d1117] border border-white/10 p-8 rounded-3xl hover:border-blue-500/40 transition-all duration-500 shadow-2xl relative overflow-hidden"
          >
            {/* Efecto de brillo sutil en el fondo */}
            <div className="absolute -right-10 -top-10 size-32 bg-blue-500/5 rounded-full blur-3xl group-hover:bg-blue-500/10 transition-all" />
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-4">
              {group.skills.map((skill, idx) => (
                <div key={idx} className="flex items-center gap-3 group/item">
                  <div className="size-10 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center group-hover/item:border-blue-500/50 group-hover/item:bg-blue-500/10 transition-all overflow-hidden">
                    
                    {skill.icon === "ts-custom" ? (
                      /* Logo oficial de TypeScript - Cuadrado Azul con 'TS' */
                      <svg viewBox="0 0 24 24" className="size-6 fill-[#007acc] group-hover/item:fill-blue-400 transition-colors">
                      ...
                      /* Iconos de Font Awesome (React, Node, etc.) */
                      <i className={`${skill.icon} text-lg text-slate-400 group-hover/item:text-blue-400 transition-colors`}></i>
                    )}

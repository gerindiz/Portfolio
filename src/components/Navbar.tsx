...
const Navbar: React.FC = () => {
  ...
  const handleAIDownload = async () => {
    ...
    setTimeout(() => { setIsOptimizing(false); setAiStatus("Descargar CV"); }, 3000);
  };

  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-300 ${
      isScrolled ? 'bg-[#212121]/80 backdrop-blur-md border-b border-white/10 py-4' : 'bg-transparent py-6'
    }`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
          <div className="size-8 bg-emerald-600 rounded-lg flex items-center justify-center font-black text-white shadow-lg shadow-emerald-500/20">G</div>
          <span className="text-white font-bold tracking-tighter hidden sm:block">
            GERMÁN <span className="text-emerald-500 text-xs uppercase tracking-widest ml-1">Rindizbacher</span>
          </span>
        </div>
        ...
        <button
          ...
          className={`group relative px-5 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 border shadow-2xl ${
            isOptimizing
              ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400 animate-pulse cursor-wait'
              : 'bg-white text-black hover:bg-emerald-600 hover:text-white border-transparent active:scale-95'
          }`}
        >
